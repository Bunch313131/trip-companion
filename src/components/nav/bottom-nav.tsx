...
export function BottomNav() {
  ...
  return (
    <nav
      className={`fixed inset-x-0 bottom-0 z-40 border-t border-border bg-surface/95 backdrop-blur-sm transition-transform duration-200 ${
        keyboardOpen ? 'translate-y-full' : 'translate-y-0'
      }`}
      style={{ paddingBottom: 'env(safe-area-inset-bottom)' }}
    >
      <div className="mx-auto flex max-w-lg justify-around px-2 pb-1.5 pt-1.5">
        {TABS.map((tab) => {
          ...
          return (
            <Link
              ...
              className={`flex flex-col items-center gap-0.5 rounded-lg px-3 py-0.5 text-[9px] font-medium transition-colors ${
